...
//        }
    ...
    public function groupAccounts(){
        ...
        $sql = "select new.id,new.username,acc.friends from zl_accounts acc LEFT JOIN zl_new_accounts new on new.username=acc.username where new.id>$start and new.id <= $end";
        $data = Db::query($sql);
        //print_r($data);//exit();
