    ...
    public function index(){
        ...
        $data[$j]['new_fri'] = $data_day_count[0]['new_fri']; $data[$j]['day_acc'] = $data_day_count[0]['day_acc'];$data[$j]['status'] = 1;
            ...
            if($data[$j]['status']==-1){
                ...
                $sql_update = "update zl_accounts set login_status = 0 from zl_accounts acc,zl_new_accounts new  where new.username=acc.username and
                new.id>$i and new.id <= ($i+80) and acc.login_status = 1 AND acc.username <> $username";
